...
def find_controls(zone_locations, zone_names, raw_text):
        ...
        for iteration, item in enumerate(verified_zone_names):
                ...
                while True:
                        ...
                        if next_sentence_location == 17616:
                                x = 1

                        next_sentence_text = raw_text[next_sentence_location]
                        
                        next_sentence_text = next_sentence_text.replace("<", " < ")
                        next_sentence_text = next_sentence_text.replace(">", " > ")

                        second_control_word_found = next_sentence_text.find('margin-left')
                        ...
                        if control_word_found == 1 or second_control_word_found != -1:
                                control_word_not_found = 0
                                control_word_found = 0
                        else:
                                control_word_not_found += 1

                        if control_word_not_found >= 70:
                                final_sentence_location = next_sentence_location - 69
                                break
                        ...
                        if int(verified_zone_locations[iteration]) != sorted_zone_locations[-1]:
                                if next_sentence_location >= sorted_zone_locations[temp_zone_index + 1]:
                                        ...
                                        if sent_diff >= 1000:
                                                final_sentence_location = current_sentence_location + 1000
                                                break
                                        elif sent_diff < 1000:
                        ...
                        if int(verified_zone_locations[iteration]) == sorted_zone_locations[-1]:
                                if control_word_not_found >= 70:
                                        final_sentence_location = current_sentence_location + 200
                                        break
