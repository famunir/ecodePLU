...
def find_accessory_uses(zone_locations, zone_names, raw_text):

        uses_by_accessory_keywords = ["ACCESSORY USES"]
        ...
        detected_accessory_keyword_location = []
        detected_accessory_names = []
        ...
        for iteration, item in enumerate(zone_names):
                ...
                if (len(raw_text) - int(current_sentence_location)) > 30:
                        ...
                        for uses_by_accessory_keyword in uses_by_accessory_keywords:
                                uses_by_accessory_keyword_check = 0
                                for sentence_iterator in range(next_sentence_location, next_sentence_location + sentences_threshold):
                                        ...
                                        uses_by_accessory_keyword_check = next_sentence_text.find(uses_by_accessory_keyword)

                                        if uses_by_accessory_keyword_check != -1:
                                                # the length of sentence in which keyword is found should be less than 150 chars.
                                                if len(str(next_sentence_text)) < 200:
                                                        accessory_uses_keyword_found = str(uses_by_accessory_keyword)
                                                        sentence_accessory_uses_keyword_found = sentence_iterator
                                                        break
                                ...
                                if uses_by_accessory_keyword_check != -1:
                                        if zone_locations[iteration] not in verified_zone_locations:  
                                                if sentence_accessory_uses_keyword_found not in detected_accessory_keyword_location:

                                                        verified_zone_locations.append(zone_locations[iteration])
                                                        verified_zone_names.append(item)
                                                        detected_accessory_keyword_location.append(sentence_accessory_uses_keyword_found)
                                                        detected_accessory_names.append(uses_by_accessory_keyword)
                                                        
        #                                                 print("\n")
        #                                                 print(item)
        #                                                 print(zone_locations[iteration])
        #                                                 print(accessory_uses_keyword_found)
        #                                                 print(sentence_accessory_uses_keyword_found)
        #                                                 print(raw_text[sentence_accessory_uses_keyword_found])
        #                                                 print("\n")
        #                                                 break
        # print(verified_zone_locations)
        # print(verified_zone_names)
        # print(detected_accessory_keyword_location)
        # print(detected_accessory_names)

        for iteration, item in enumerate(verified_zone_names):
                current_zone_name = str(item)
                accessory_right_keyword = detected_accessory_names[iteration]
                current_sentence_location = detected_accessory_keyword_location[iteration]
                ...
                uses_dict["Accessory uses"] = plu_text
                all_accessory_uses[raw_text[int(verified_zone_locations[iteration])]] = uses_dict
                #print("\n")
