...
def find_conditional_uses(zone_locations, zone_names, raw_text):

        uses_by_conditional_keywords = ["CONDITIONAL USES"]

        # other_uses_words = ["conditional uses", "conditional use"]

        # all conditional uses for all the zones in the city
        all_conditional_uses = {}

        detected_conditional_keyword_location = []
        detected_conditional_names = []
        ...
        for iteration, item in enumerate(zone_names):
                ...
                if (len(raw_text) - int(current_sentence_location)) > 30:
                        ...
                        # Loop for "conditional uses" keyword
                        for uses_by_conditional_keyword in uses_by_conditional_keywords:
                                uses_by_conditional_keyword_check = 0
                                for sentence_iterator in range(next_sentence_location, next_sentence_location + sentences_threshold):
                                        ...
                                        uses_by_conditional_keyword_check = next_sentence_text.find(uses_by_conditional_keyword)

                                        if uses_by_conditional_keyword_check != -1:
                                                # the length of sentence in which keyword is found should be less than 150 chars.
                                                if len(str(next_sentence_text)) < 200:
                                                        conditional_uses_keyword_found = str(uses_by_conditional_keyword)
                                                        sentence_conditional_uses_keyword_found = sentence_iterator
                                                        break
                                ...
                                if uses_by_conditional_keyword_check != -1:
                                        if zone_locations[iteration] not in verified_zone_locations:  
                                                if sentence_conditional_uses_keyword_found not in detected_conditional_keyword_location:

                                                        verified_zone_locations.append(zone_locations[iteration])
                                                        verified_zone_names.append(item)
                                                        detected_conditional_keyword_location.append(sentence_conditional_uses_keyword_found)
                                                        detected_conditional_names.append(uses_by_conditional_keyword)
                                                        
        #                                                 print("\n")
        #                                                 print(item)
        #                                                 print(zone_locations[iteration])
        #                                                 print(conditional_uses_keyword_found)
        #                                                 print(sentence_conditional_uses_keyword_found)
        #                                                 print(raw_text[sentence_conditional_uses_keyword_found])
        #                                                 print("\n")
        #                                                 break
        # print(verified_zone_locations)
        # print(verified_zone_names)
        # print(detected_conditional_keyword_location)
        # print(detected_conditional_names)

        for iteration, item in enumerate(verified_zone_names):
                current_zone_name = str(item)
                conditional_right_keyword = detected_conditional_names[iteration]
                current_sentence_location = detected_conditional_keyword_location[iteration]
                ...
                uses_dict["Conditional uses"] = plu_text
                all_conditional_uses[raw_text[int(verified_zone_locations[iteration])]] = uses_dict
                #print("\n")
